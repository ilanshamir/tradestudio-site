...
description: "Tradevoy is a native desktop app for Windows, macOS, and Linux. That's not legacy thinking — it's a deliberate architectural choice for performance-critical trading analysis."
date: 2026-02-10
...
Tradevoy renders charts with SkiaSharp — the same graphics library that powers Google Chrome's rendering engine, but called natively from C#. Hardware-accelerated. No browser sandbox. No JavaScript event loop.
...
Tradevoy compiles C# strategies in-process using Roslyn. This isn't possible in a browser. You can't run the .NET compiler in a web page. You'd need to send code to a server, compile there, and send results back.
...
AvalonDock gives Tradevoy native window docking — tear off panels, snap to zones, create any layout. Browser-based docking libraries exist, but they're fighting the browser's layout engine the entire time.
...
- **Desktop only.** No mobile, no iPad. Our users are traders sitting at desks. If you need charts on your phone, Tradevoy isn't for that.
...
[Download Tradevoy](/download) and open a chart. Pan around. Zoom in and out. Add a few indicators. Compare it to your current web-based platform.
